...
class DatabaseDataGenerator:
    def __init__(self):
        ...
        self.TARGET_RECORDS_PER_TABLE = 10_000
    ...
    def generate_users(self, count: int = 1_000_000):
        ...
        for i in range(count):
            ...
            # Generate phone number and ensure max 20 chars, remove spaces and dashes
            phone = self.fake.phone_number()
            phone = ''.join(filter(str.isdigit, phone))[:20]  # keep only digits, max 20 chars
    ...
    def generate_products(self, count: int = 1_000_000):
        ...
        # Find the highest existing SKU number in the database
        try:
            self.cursor.execute("SELECT sku FROM products WHERE sku ~ '^SKU-\\d{8}$' ORDER BY sku DESC LIMIT 1;")
            row = self.cursor.fetchone()
            if row and row[0]:
                last_sku = int(row[0].replace("SKU-", ""))
                sku_start = last_sku + 1
            else:
                sku_start = 0
        except Exception as e:
            print(f"⚠️ Could not determine last SKU, defaulting to 0: {e}")
            sku_start = 0
    ...
    def generate_orders(self, count: int = 2_000_000):
        ...
        # Find the highest existing order_number in the database
        try:
            self.cursor.execute("SELECT order_number FROM orders WHERE order_number ~ '^ORD-\\d{8}$' ORDER BY order_number DESC LIMIT 1;")
            row = self.cursor.fetchone()
            if row and row[0]:
                last_ord = int(row[0].replace("ORD-", ""))
                ord_start = last_ord + 1
            else:
                ord_start = 0
        except Exception as e:
            print(f"⚠️ Could not determine last order_number, defaulting to 0: {e}")
            ord_start = 0
        ...
        for i in range(count):
            ...
            while True:
                order_number = f"ORD-{ord_start + i:08d}"
    ...
    def run_generation(self):
        ...
        try:
            ...
            self.generate_product_sizes(30_000)
            self.generate_carts(8000)
